...
export default class ASCIIEncoder implements Encoder {
  ...
  public encode(context: EncoderContext): void {
    ...
    if (n >= 2) {
      context.writeCodeword(ASCIIEncoder.encodeASCIIDigits(StringUtils.getCharCode(context.getMessage().charAt(context.pos)),
        StringUtils.getCharCode(context.getMessage().charAt(context.pos + 1))));
      context.pos += 2;
    } else {
